...
twitter_auth_keys = {
	"consumer_key"        : os.environ.get("ABDOU_BOT_CONSUMER_KEY"),
	"consumer_secret"     : os.environ.get("ABDOU_BOT_CONSUMER_SECRET"),
	"access_token"        : os.environ.get("ABDOU_BOT_ACCESS_TOKEN"),
	"access_token_secret" : os.environ.get("ABDOU_BOT_ACCESS_TOKEN_SECRET")
}
...
with open( join(dirname(__file__), 'gsheets_api-credentials.json'),'w') as f:
    json.dump(json.loads(os.environ.get("ABDOU_BOT_GDRIVE_API_CREDENTIALS")),f)

gc = pygsheets.authorize(service_file=join(dirname(__file__), 'gsheets_api-credentials.json'))

sht = gc.open_by_key(os.environ.get("ABDOU_BOT_SHEET_KEY"))
